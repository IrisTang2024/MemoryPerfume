// Backend server for Memory Fragrance
...
// Initialize OpenAI with verification
let openaiClient;
...
app.post('/api/analyze-image', upload.single('image'), async (req, res) => {
  try {
    ...
    console.log(`Processing image upload: ${req.file.originalname} (${req.file.size} bytes, ${req.file.mimetype})`);
    ...
    3. COMPOSITION:
       - Top Notes: List only 3-4 premium ingredient names, separated by commas (no descriptions)
       - Heart Notes: List only 3-4 premium ingredient names, separated by commas (no descriptions)
       - Base Notes: List only 2-3 premium ingredient names, separated by commas (no descriptions)
       - Main Accord: A single refined fragrance family descriptor (e.g., "Ambery Oriental," "Fresh Aromatic")
    
    4. PERFUMER'S REVIEW: A poetic, technical description (60-80 words) using refined perfumery vocabulary. Focus on scent progression, sillage, and longevity. Use sophisticated technical terms like "aldehydic," "petrichor," "ambroxan," or "chypre structure" when appropriate. Reference historical perfumery traditions or techniques.
    
    Format your response exactly as structured above with clear section breaks. Each section must be on separate lines with precise formatting. Do not combine sections or run them together. Start each section with its label (NAME:, PHOTO INSPIRATION:, etc.) followed by the content. Use the exact dash format for notes (e.g., "- Top Notes:").
    
    Ensure your descriptions evoke luxury, exclusivity, craftsmanship, and exceptional quality throughout.
    `;
